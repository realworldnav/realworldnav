...
# Define Excel styles (matching original Excel generator)
font_name = "Inter"
...
def create_header_section(ws, fund_name: str, report_date: datetime, start_date: datetime, 
                         currency: str = "ETH", sheet_title: str = "Financial Report"):
    """Create standardized header section for all sheets"""
    # Header styling
    ...
    ws["E3"] = f"Start of period {start_date.strftime('%m/%d/%Y')}"
...
def create_account_statement_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    """Create Account Statement sheet (Income Statement)"""
    ws = wb.create_sheet(title="Account Statement")
    
    start_date = report_date.replace(day=1)
    create_header_section(ws, fund_name, report_date, start_date, currency, 
                         "Account statement (unaudited)")
    
    # Check if GL data has required columns
    if gl_df.empty or 'date' not in gl_df.columns:
        ws["A6"] = "No valid GL data available for account statement"
        return
    ...
    try:
        ...
        ws["A6"] = f"Error generating income/expense data: {str(e)}"
        return
    
    # Section title
    row = 6
    ws.cell(row=row, column=1, value="Statement of income")
    ws.cell(row=row, column=1).font = Font(name=font_name, color="0047BA", bold=True)
    ws.cell(row=row, column=1).alignment = left
    
    # Table headers
    row += 1
    ws.cell(row=row, column=1, value="Income")
    ws.cell(row=row, column=1).font = font_black
    ws.cell(row=row, column=1).alignment = left
    ...
    if not income_df.empty:
        for idx, (_, row_data) in enumerate(income_df.iterrows()):
            fill = alt_fill if idx % 2 == 1 else no_fill
            ws.cell(row=row, column=1, value="  " + row_data['GL_Acct_Name']).font = font_black
            ws.cell(row=row, column=1).alignment = left
            ws.cell(row=row, column=1).fill = fill
            
            for j, period in enumerate(['MTD', 'QTD', 'YTD', 'ITD']):
                val = row_data.get(period, 0)
                cell = ws.cell(row=row, column=2 + j, value=val)
                cell.font = font_black
                cell.alignment = right
                cell.fill = fill
                cell.number_format = "#,##0.000000_);(#,##0.000000)" if currency == 'ETH' else "#,##0.00_);(#,##0.00)"
                income_totals[j] += val
            row += 1
    ...
    if not expense_df.empty:
        for idx, (_, row_data) in enumerate(expense_df.iterrows()):
            fill = alt_fill if idx % 2 == 1 else no_fill
            ws.cell(row=row, column=1, value="  " + row_data['GL_Acct_Name']).font = font_black
            ws.cell(row=row, column=1).alignment = left
            ws.cell(row=row, column=1).fill = fill
...
def create_nav_changes_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    start_date = report_date.replace(day=1)
    create_header_section(ws, fund_name, report_date, start_date, currency,
                         "Statement of changes in net asset value (unaudited)")
    
    # Check if GL data has required columns
    if gl_df.empty or 'date' not in gl_df.columns:
        ws["A6"] = "No valid GL data available for NAV changes"
        return
    ...
    try:
        ...
        ws["A6"] = f"Error generating NAV data: {str(e)}"
        return
    
    if nav_df.empty:
        ws["A6"] = "No NAV data available"
        return
    ...
    # Headers starting from column 1 (no blank first column)
    headers = ["Month to Date", "Quarter to Date", "Year to Date", "Inception to Date"]
    ...
    for i, item in enumerate(nav_items):
        ...
        for period_idx, period in enumerate(['Month to Date', 'Quarter to Date', 'Year to Date', 'Inception to Date']):
            period_row = nav_df[nav_df['Period'] == period]
...
def create_trial_balance_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    create_header_section(ws, fund_name, report_date, comp_date, currency,
                         "Trial balance (unaudited)")
    
    # Check if GL data has required columns
    if gl_df.empty or 'date' not in gl_df.columns:
        ws["A6"] = "No valid GL data available for trial balance"
        return
    ...
    try:
        ...
        ws["A6"] = f"Error generating trial balance data: {str(e)}"
        return
...
def create_assets_liabilities_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    create_header_section(ws, fund_name, report_date, report_date, currency,
                         "Statement of assets and liabilities (unaudited)")
    
    # Check if GL data has required columns
    if gl_df.empty or 'date' not in gl_df.columns:
        ws["A6"] = "No valid GL data available for assets & liabilities"
        return
    ...
    # Apply bad debt netting logic (matching original Excel generator)
...
def create_operating_expenses_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    # Check if GL data has required columns
    if gl_df.empty or 'date' not in gl_df.columns:
        ws["A8"] = "No valid GL data available for expense schedule"
        return
    ...
    try:
        ...
        ws["A8"] = f"Error generating trial balance data: {str(e)}"
        return
    ...
    for col in range(1, len(headers) + 1):
        ...
        if col_letter == "A":
            ...
            max_len = max(len(str(ws.cell(row=r, column=col).value or "")) for r in range(7, row + 1))
...
def create_management_fee_sheet(wb, gl_df: pd.DataFrame, fund_name: str, 
    ...
    if gl_df.empty or 'account_name' not in gl_df.columns:
    ...
    try:
        ...
        # Filter management fee expenses from GL (using regex to match with underscores or spaces)
        ...
        for _, lp_row in lp_df.iterrows():
            try:
                ...
                # For simplified calculation, use total management fee expense from GL
                if currency == 'ETH':
                    ...
                    # For USD, would need ETH price conversion - using simplified version
                ...
                # For simplicity, divide equally among LPs (in reality this would be calculated per LP)
                lp_count = len(lp_df)
        ...
        ws.cell(row=row, column=4).number_format = "#,##0.000000_);(#,##0.000000)" if currency == 'ETH' else "#,##0.00_);(#,##0.00)"
        ...
        ws.cell(row=row, column=6).number_format = "#,##0.000000_);(#,##0.000000)" if currency == 'ETH' else "#,##0.00_);(#,##0.00)"
...
def generate_excel_report(gl_df: pd.DataFrame, fund_name: str, report_date: datetime, 
    ...
    """
    Generate complete Excel report with all sections
    ...
    create_nav_changes_sheet(wb, gl_df, fund_name, report_date, currency)
